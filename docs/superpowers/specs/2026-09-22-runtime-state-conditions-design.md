...
本阶段只交付 `app/agent_runtime/state.py` 与 `conditions.py`，不启动 Graph、Runner、Tool 或 Agent。继续使用公共 `AgentRunState` 作为唯一状态模型。`new_run(run_id, user_message)` 创建待运行状态；`update_run(state, **changes)` 原子校验整份新状态，返回新实例，不改原实例，不允许改写 run ID、原始消息或 schema 版本。替换已有 Brief、Evidence 或 Proposal 内容时，即使 ID 未变，也须在同一次更新中清除下游结果；Brief 变化还须重置规划轮次。首次写入可原子设置相互匹配的字段。公共契约校验整体结构，状态入口校验旧内容变化引起的失效。
